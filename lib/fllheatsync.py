...
# 热度榜
def heatRank1(env,dbdict,aType='1'):
    '''
    aType:爬虫type类型
    ...
    OrigData = getOrigData(env,dbdict,src_table)
    if OrigData:
        rankList = OrigData
        for d in rankList:
            starName = d["starName"]
...
#从分析库获取数据源
def getOrigData(env,dbdict,db_table=''):
    """
    data_type  1实时 2周 3月
    ...
    if db_table == "heat_statistics_hour":
        ...
        key_value=db.queryOne(key_sql)["cfg_value"]
        sql = """select * from %s where analysis_task_ticket='%s';"""%(db_table,key_value)
    elif db_table == "heat_statistics_month":
        key_sql = '''select cfg_value from business_config where cfg_name='HOT_REAL-TIME_BATCHES_MONTH';'''
        key_value=db.queryOne(key_sql)["cfg_value"]
        sql = """select * from %s where analysis_task_ticket='%s';"""%(db_table,key_value)
...
def main():
    ...
    for env in dbdict.keys():
        ...
        if aType == '1':
            ...
            '''
            heatRank1(env,env_dbdict,aType='1')
        #周榜
        elif aType == '2':
            '''
            微舆情热度榜
            ...
            '''
            heatRank1(env,env_dbdict,aType='2')
        #月榜
        elif aType == '3':
            '''
            微舆情热度榜
            ...
            dst:192.168.20.101|super_fans_beta|tb_star_hot|(data_type:2)
            '''
            heatRank1(env,env_dbdict,aType='3')
