...
def starRank(env,dbdict, src_aType, dType):
    ...
    starSql = """select starName,weibo_screen_name from tb_star where (type=0 or type=1 or type=3 or type=5 or type=6 or type=7 or type=9 or type=10) and add_type=1;"""
    ...
    if OrigData:
        ...
        for d in rankList:
            ...
            if starName in starName_list:
                ...
                #positive
                positiveNum = d["positiveNum"]
                positiveScore = d["positiveScore"]
                positiveRank = d["positiveRank"]
                ...
                if len(searchResults) == 0 :
                    insertSql = """INSERT INTO tb_star_power (starName,searchNum,score,loveRank,searchRank,discussRank,
                                   ineractRank,loveNum,rankNumber,ineractNum,discussNum,ineractScore,loveScore,discussScore,searchScore,positiveNum,positiveRank,positiveScore,data_type,date,score_type,create_time,capture_time) VALUES ('%s',
                                   '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');"""%(realName,searchNum,score,
                                   loveRank,searchRank,discussRank,ineractRank,loveNum,rankNumber,ineractNum,discussNum,ineractScore,loveScore,discussScore,searchScore,positiveNum,positiveRank,positiveScore,data_type,current_date,dst_aType,current_time,capture_time)
                    logger.info(insertSql)
                    db.executeSql(insertSql)
                    # db.connection.commit()#执行commit操作，插入语句才能生效
                    logger.info("明星：%s 已插入"%starName)
                else:
                    updateSql = """UPDATE tb_star_power SET starName='%s',searchNum='%s',score='%s',loveRank='%s',searchRank='%s',
                                   discussRank='%s',ineractRank='%s',loveNum='%s',rankNumber='%s',ineractNum='%s',discussNum='%s',ineractScore='%s',loveScore='%s',discussScore='%s',searchScore='%s',positiveNum='%s',positiveRank='%s',positiveScore='%s',score_type='%s',
                                   update_time='%s',capture_time='%s' where starName='%s' and data_type='%s' and date='%s';"""%(realName,searchNum,score,loveRank,searchRank,discussRank,ineractRank,
                                                         loveNum,rankNumber,ineractNum,discussNum,ineractScore,loveScore,discussScore,searchScore,positiveNum,positiveRank,positiveScore,dst_aType,current_time,capture_time,realName,data_type,current_date)
...
def main():
    ...
    for env in dbdict.keys():
        ...
        #周榜
            ...
            '''
            area_type = ['0-week','1-week', '3-week', '5-week','6-week','7-week']
            for aType in area_type:
        ...
        #月榜
            ...
            '''
            area_type = ['0-month','1-month', '3-month', '5-month','6-month','7-month']
            for aType in area_type:
